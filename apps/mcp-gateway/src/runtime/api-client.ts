...
export type AiProviderTelemetryQuery = {
  projectId: string;
  limit?: number;
};

export type RuntimeDiagnosticsQuery = {
  projectId: string;
  limit?: number;
};
...
export class GatewayApiClient {
  ...
  async fetchApiHealth(): Promise<Record<string, unknown>> {
    const response = await this.fetchImpl(`${this.dependencies.apiBaseUrl}/health`, {
      method: "GET",
    });
  ...
  async fetchAiProviderHealth(bearerToken: string): Promise<Record<string, unknown>> {
    const response = await this.fetchImpl(
      `${this.dependencies.apiBaseUrl}/v1/ai/provider-health`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      throw new AppError(
        ...
        (payload as { error?: { code?: string } }).error?.code ??
          "UPSTREAM_REQUEST_FAILED",
        (payload as { error?: { message?: string } }).error?.message ??
          "Failed to load AI provider health",
      );
    }
  ...
  async searchKnowledgeEntries(
    ...
    const response = await this.fetchImpl(
      `${this.dependencies.apiBaseUrl}/v1/projects/${query.projectId}/knowledge-search?${this.buildQuery({
        q: query.q,
        sourceType: query.sourceType,
        stageCode: query.stageCode,
        limit: query.limit?.toString(),
  ...
  async fetchAiProviderTelemetry(
    query: AiProviderTelemetryQuery,
    bearerToken: string,
  ): Promise<Record<string, unknown>> {
    const response = await this.fetchImpl(
      `${this.dependencies.apiBaseUrl}/v1/projects/${query.projectId}/ai/provider-telemetry?${this.buildQuery({
    ...
    if (!response.ok) {
      throw new AppError(
        ...
        (payload as { error?: { code?: string } }).error?.code ??
          "UPSTREAM_REQUEST_FAILED",
        (payload as { error?: { message?: string } }).error?.message ??
          "Failed to load AI provider telemetry",
      );
    }
