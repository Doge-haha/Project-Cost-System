...
- [ ] 为 `project.project_code` 建唯一索引
- [ ] 为 `project_stage(project_id, stage_code)` 建唯一索引
- [ ] 为 `project_member(project_id, user_id)` 建唯一索引
- [ ] 为 `project_discipline(project_id, discipline_code)` 建唯一索引
- [ ] 为 `discipline_type.discipline_code` 建唯一索引
- [ ] 为 `standard_set.standard_set_code` 建唯一索引
...
- [ ] 初始化一版专业主数据
- [ ] 初始化一版定额集主数据
- [ ] 预留江苏源系统字段：`source_field_code`, `source_markup`, `source_system`
...
- [ ] 固化项目状态枚举：`draft`, `in_progress`, `under_review`, `archived`
- [ ] 固化阶段状态枚举：`not_started`, `in_progress`, `pending_review`, `approved`, `completed`, `skipped`
- [ ] 固化平台角色枚举
- [ ] 固化业务身份枚举
- [ ] 固化资源类型枚举：`project`, `stage`, `project_discipline`, `standard_set`
- [ ] 固化操作类型枚举：`view`, `edit`, `submit`, `review`, `import`
...
- [ ] 创建项目时自动展开默认阶段模板
- [ ] 创建项目时写入默认 `project_stage`
- [ ] 创建项目时将负责人写入 `project_member`
- [ ] 创建项目时支持绑定默认价目版本和默认取费模板
...
- [ ] 实现 `PUT /api/v1/projects/{id}/disciplines`
- [ ] 实现 `GET /api/v1/standard-sets`

### 3.7.2 业务规则

- [ ] 校验 `discipline_code` 必须存在于 `discipline_type`
- [ ] 校验 `standard_set_code` 必须存在于 `standard_set`
- [ ] 校验默认定额集与专业类型匹配
- [ ] 支持项目专业排序
- [ ] 支持项目专业启停
...
- [ ] 导入江苏专业主数据样本
- [ ] 导入江苏定额集样本
- [ ] 保留源字段：`source_field_code`, `source_markup`, `source_system`
...
- [ ] 固化 `discipline_type` 的源字段结构
- [ ] 固化 `standard_set` 的源字段结构
- [ ] 确认 `source_system = xindian_jiangsu`
- [ ] 预留导入任务表或导入日志表设计
