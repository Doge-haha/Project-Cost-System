...
- [x] `bill_version` 加入 `source_spec_code`
- [x] `bill_version` 加入 `source_spec_name`
- [x] `bill_version` 加入 `source_visible_flag`
- [x] `bill_version` 加入 `source_default_flag`
- [x] `bill_item` 加入 `source_bill_id`
- [x] `bill_item` 加入 `source_sequence`
- [x] `bill_item` 加入 `source_level_code`
- [x] `bill_item` 加入 `is_measure_item`
- [x] `bill_item` 加入 `source_reference_price`
- [x] `bill_item` 加入 `source_fee_id`
- [x] `bill_item` 加入 `measure_category`
- [x] `bill_item` 加入 `measure_fee_flag`
- [x] `bill_item` 加入 `measure_category_subtype`
- [x] `bill_item_work_item` 加入 `source_spec_code`
- [x] `bill_item_work_item` 加入 `source_bill_id`

### 3.1.3 索引与约束

- [x] 为 `bill_version(project_id, stage_id, version_no)` 建唯一索引
- [x] 为 `bill_item(bill_version_id, item_code, parent_id)` 建业务唯一索引
- [x] 为 `bill_item_work_item(bill_item_id, sort_order)` 建唯一索引
- [x] 建立 `bill_item.parent_id -> bill_item.id` 外键
- [x] 建立 `bill_item_work_item.bill_item_id -> bill_item.id` 外键
...
- [x] 固化清单版本类型枚举：`initial`, `reference_copy`, `contract_baseline`, `change`, `settlement`
- [x] 固化清单版本状态枚举：`editable`, `submitted`, `approved`, `locked`, `rejected`
- [x] 固化清单锁定状态枚举：`unlocked`, `lock_requested`, `locked`, `unlock_requested`
- [x] 固化清单校验状态枚举：`normal`, `warning`, `error`
...
- [x] 实现从上游版本创建 `reference_copy`
- [x] 实现记录 `source_stage_id`
- [x] 实现记录 `source_version_id`
- [x] 实现保留来源展示字段
- [x] 实现分叉后新版本默认进入 `editable`
...
- [x] 实现 `bill_item` 仓储
- [x] 支持树形父子关系查询
- [x] 支持同级 `sort_order` 排序

### 3.4.2 业务层

- [x] 按 `item_code` 和 `parent_id` 建树
- [x] 支持章、节、清单项三级或多级展示
- [x] 支持根节点批量创建
- [x] 支持子节点插入
- [x] 支持节点移动后的排序重排
...
- [x] 实现 `GET /api/v1/projects/{id}/bill-items/{itemId}/work-items`
- [x] 实现 `POST /api/v1/projects/{id}/bill-items/{itemId}/work-items`
- [x] 实现 `PUT /api/v1/projects/{id}/bill-items/{itemId}/work-items/{workItemId}`

### 3.5.2 业务规则

- [x] 工作内容按 `sort_order` 排序
- [x] 工作内容必须归属一个有效清单项
- [x] 锁定版本下禁止编辑工作内容
- [x] 父清单项删除或失效时同步处理工作内容
...
- [x] 增加 `GET /api/v1/projects/{id}/bill-versions`
- [x] 增加 `POST /api/v1/projects/{id}/bill-versions`
- [x] 增加 `POST /api/v1/projects/{id}/bill-versions/{versionId}/copy-from`
- [x] 增加 `GET /api/v1/projects/{id}/bill-versions/{versionId}/source-chain`

### 3.6.3 过滤与查询能力

- [x] 支持按 `stageCode` 查询清单
- [x] 支持按 `billVersionId` 查询清单
- [x] 支持按 `disciplineCode` 查询清单
- [x] 支持按关键字搜索 `item_code` / `item_name`
