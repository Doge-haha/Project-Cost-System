...
- [ ] 为 `price_version.version_code` 建唯一索引
- [ ] 为 `price_item(price_version_id, quota_code)` 建唯一索引
...
- [ ] 固化定额来源方式枚举：`manual`, `ai`, `history_reference`
- [ ] 固化价目版本状态枚举：`draft`, `active`, `inactive`
- [ ] 固化取费模板状态枚举：`draft`, `active`, `inactive`
- [ ] 固化计价字段口径：`system_*`, `manual_*`, `final_*`
- [ ] 固化定额校验状态枚举：`normal`, `warning`, `error`
