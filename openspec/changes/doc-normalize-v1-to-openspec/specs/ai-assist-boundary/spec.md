...
- `apps/worker` 当前已有 `knowledge-extraction-worker`，可执行知识抽取预览类任务。
- 当前仓库的 AI 能力更接近“runtime 预览和知识抽取骨架”，完整的推荐接受/忽略/失效与正式落库主链仍主要体现在文档和迭代拆分中。
- 架构文档已明确 AI Runtime 不负责正式业务写入，正式写入必须通过 `apps/api`。

这些现状说明 AI 边界已经在架构层被确定，但完整产品化能力仍属于后续工程工作。
