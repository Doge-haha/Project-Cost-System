...
### Requirement: MUST V1 的 AI 能力必须聚焦辅助推荐、预警分析和数据沉淀

MUST 在 V1 范围内，AI 能力必须以辅助推荐、预警分析和数据沉淀为主，不应把后续增强能力误写成首版强制交付。
...
### Requirement: MUST AI 输出必须作为建议而非直接正式写入

MUST AI 生成的结果必须首先作为建议、预警或解释信息存在，不能绕过正式业务链直接写入主业务数据。
...
### Requirement: MUST 正式业务写入只能通过业务后端完成

MUST 无论 AI Runtime、Worker 还是其他辅助能力层，都不能绕过业务后端直接执行正式业务写入。
...
### Requirement: MUST AI 结果必须支持人工确认

MUST AI 推荐、预警或辅助审核结果必须支持人工确认、接受、忽略或失效等显式处理动作。
...
### Requirement: MUST AI 结果必须受权限控制

MUST AI 结果的查看、接受、忽略和落正式数据动作必须受角色和项目权限控制，不得对所有用户开放同样能力。
...
### Requirement: MUST AI 结果必须具备失效机制

MUST 当源上下文、清单版本、定额上下文或其他关键条件发生变化时，旧的 AI 建议必须允许失效，避免继续误导业务操作。
...
### Requirement: MUST AI 辅助审核必须输出依据和解释

MUST AI 在辅助审核、异常识别和预警场景中，必须输出支撑理由、依据摘要或上下文说明，而不仅是单一结论。
...
### Requirement: MUST AI 读上下文可以广，但正式写入边界必须窄

MUST AI 可以在权限允许前提下读取项目、阶段、清单、知识或历史案例上下文，但正式写入边界必须严格受限。
...
### Requirement: MUST AI 关键动作必须保留审计与追溯

MUST AI 推荐生成、接受、忽略、失效和人工确认落正式数据等关键动作，必须保留审计记录。
