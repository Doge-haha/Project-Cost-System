...
### Requirement: AI Copilot 只能作为辅助交互层存在

AI Copilot 的定位必须是辅助推荐、预警和解释，而不是绕过业务规则直接执行正式写操作。
...
### Requirement: AI 推荐不得直接改写正式业务表

AI 推荐在未经过人工确认前，不得直接写入正式清单、定额或审核结论。
...
### Requirement: AI 交互必须支持人工接受、忽略和失效

AI 推荐结果必须支持人工接受、忽略和失效等状态流转，以反映人工判断和上下文过期。
...
### Requirement: AI 失败必须降级而不阻断人工主流程

AI 调用失败、超时或返回异常时，不得阻断人工主流程，应提示稍后重试并允许继续手工处理。
...
### Requirement: AI Copilot 必须提供明确上下文来源

每次 AI 交互都必须基于明确上下文来源，例如清单特征、定额候选、偏差结果或审计日志，而不是无约束自由生成。
