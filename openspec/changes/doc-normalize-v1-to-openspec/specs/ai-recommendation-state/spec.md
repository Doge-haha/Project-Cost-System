...
### Requirement: MUST AI 推荐必须从 generated 起始

MUST AI 推荐一旦生成，初始状态必须是 `generated`，不能跳过待人工判断阶段直接进入终态。
...
### Requirement: MUST 仅人工确认才能进入 accepted

MUST 只有经过人工明确确认的推荐结果，才能从 `generated` 进入 `accepted`。
...
### Requirement: MUST 人工忽略必须进入 ignored

MUST 当用户决定不采用某推荐时，系统必须将该推荐置为 `ignored`，而不是简单删除。
...
### Requirement: MUST 上下文变化必须使旧推荐进入 expired

MUST 当源清单版本、定额上下文、阶段或相关输入条件发生关键变化时，旧推荐必须失效并进入 `expired`。
...
### Requirement: MUST 终态推荐不得被直接改写回 generated

MUST 推荐进入 `accepted`、`ignored` 或 `expired` 之后，不得再直接改写回 `generated`，如需重新建议应生成新推荐记录。
...
- 当前状态机文档和迭代任务已明确 `generated -> accepted / ignored / expired` 主链，并补充 `accepted -> rolled_back` 撤销链路。
