...
### Requirement: MUST OpenAPI必须作为接口契约基线

MUST 系统必须以 OpenAPI 文档作为前后端联调和接口边界的契约基线，而不是依赖口头约定或代码猜测。
...
### Requirement: MUST API必须采用统一的/v1版本前缀

MUST 正式业务 API 必须采用统一的 `/v1` 版本前缀，以明确版本边界。
...
### Requirement: MUST API必须按业务资源分组而不是按页面分组

MUST API 应优先按项目、阶段、清单版本、审核提交、过程单据、报表和任务等业务资源分组，而不是按前端页面结构分组。
...
### Requirement: MUST 写接口必须使用明确动作语义

MUST 需要触发业务动作的接口必须使用清晰语义，例如提交、审核、复制来源、撤回、重算、导出任务等，而不是把所有动作揉进模糊更新接口。
...
### Requirement: MUST 请求和响应结构必须使用稳定schema

MUST 每个正式接口都必须对应稳定的请求和响应 schema，避免同一接口在不同调用场景返回不兼容结构。
...
### Requirement: MUST 错误返回必须采用统一结构

MUST 系统错误返回必须至少包含错误码和错误消息，并允许携带结构化细节。
...
### Requirement: MUST API必须区分同步查询和异步任务接口

MUST 同步查询接口和异步任务接口必须区分开来，涉及导出、后台处理和长时间执行任务时应使用任务型接口。
...
### Requirement: MUST API契约必须与权限模型协同设计

MUST API 最低权限要求必须能够与权限模型映射，不得出现接口契约与权限矩阵完全脱节的情况。
