...
### Requirement: 所有关键状态切换必须写入审计日志

项目、阶段、清单版本、审核流、过程单据、导出任务和 AI 接受类关键动作都必须写入审计日志。
...
审计日志必须能关联不同类型资源，而不是为每类资源单独维护互不统一的日志结构。
...
### Requirement: 审计日志必须保留操作者和时间

每条正式审计日志都必须保留操作者、项目上下文和创建时间，以支撑追溯和责任判定。
...
### Requirement: 审计日志应支持前后状态快照

对更新类动作，审计日志应尽量保留变更前和变更后的结构化快照，而不是只留一段描述性文本。
...
### Requirement: 审计日志必须支持按资源和时间过滤查询

审计日志查询必须至少支持按资源类型、资源标识、动作、操作者和时间范围过滤。
...
### Requirement: 审计日志必须受项目可见权限控制

审计日志虽然用于追溯，但仍必须受项目可见范围和日志查看权限约束，不能向无权用户暴露。
