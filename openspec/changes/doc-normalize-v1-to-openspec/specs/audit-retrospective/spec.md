...
### Requirement: MUST 项目复盘必须作为正式阶段承接结果回收

MUST 项目复盘必须作为正式业务阶段存在，用于承接结算后的差异分析、经验总结和资产沉淀。
...
### Requirement: MUST 复盘必须基于可追溯事实而不是纯文本总结

MUST 正式复盘结论必须建立在版本链、审核记录、审计日志、偏差结果和过程单据等可追溯事实之上。
...
### Requirement: MUST 复盘必须支持指标提取和模式归纳

MUST 项目复盘不应只产出报告文本，还必须支持提取结构化指标、差异模式和经验规则。
...
### Requirement: MUST 复盘知识必须进入统一知识沉淀链路

MUST 复盘得到的知识结论必须进入统一知识沉淀链路，与审核经验、异常模式和 AI 反馈一起复用。
...
### Requirement: MUST 复盘结果必须服务后续项目引用

MUST 项目复盘的目标之一是支持后续项目复用，因此复盘结果必须能被检索、过滤和引用，而不是只保存在单项目附件里。
...
- 知识抽取结果持久化后会同步写入知识/记忆生成审计日志，复盘和审核经验沉淀具备可追溯基础。
- 项目状态更新接口会同步写入 `project.update_status` 审计日志，可追溯项目状态前后值。
- 清单提交、撤回、锁定、解锁以及审核提交、通过、驳回、撤回引起的阶段状态变化已同步写入 `project_stage` 审计日志，可追溯阶段状态前后值和触发业务资源。
- 定额行新增、修改、删除以及清单项计价计算、人工调价、批量重算已写入审计日志，计价结果变动具备复盘依据。
