...
### Requirement: MUST 长耗时能力必须通过后台任务承接

MUST 报表导出、批量重算、导入处理、知识抽取、AI 推荐和其他长耗时能力必须通过后台任务承接，不得阻塞交互请求等待完整执行完成。
...
### Requirement: MUST 后台任务必须具备统一状态机

MUST 后台任务必须至少支持 `queued`、`processing`、`completed`、`failed` 四类状态，并保持单向状态流转。
...
### Requirement: MUST 任务记录必须保留可追踪元数据

MUST 每条后台任务必须记录任务类型、所属项目、请求人、payload、状态、结果、错误信息、创建时间、开始时间和完成时间等追踪字段。
...
### Requirement: MUST Worker必须通过明确领取接口获取任务

MUST Worker 必须通过明确的领取或消费接口获取待执行任务，前端查询任务状态不得改变任务状态。
...
### Requirement: MUST 任务执行结果必须由执行方回写

MUST 任务完成或失败时，执行方必须回写结果或错误信息，任务总线不得只依赖前端推断执行结果。
...
### Requirement: MUST 失败任务必须支持受控重试

MUST 失败任务必须支持受控重试或重新发起，重试时必须明确复用原始输入、修正输入或失败子集输入，不得静默混用不同范围。
...
### Requirement: MUST 任务类型必须有明确执行器归属

MUST 每类后台任务必须有明确的处理器或执行器归属，未知任务类型不得被静默吞掉。
...
### Requirement: MUST 关键任务状态变化必须写入审计日志

MUST 后台任务的入队、完成、失败和关键重试动作必须写入审计日志或等价追踪记录。
...
### Requirement: MUST 后台任务查询必须受权限控制

MUST 后台任务查询、下载结果和重试操作必须受项目可见性、任务所有权和对应业务权限控制。
