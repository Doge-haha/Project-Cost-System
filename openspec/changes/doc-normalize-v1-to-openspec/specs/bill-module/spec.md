...
### Requirement: 清单模块必须以版本为核心上下文

清单模块的所有读取、编辑、引用和提交动作都必须围绕清单版本上下文进行，不应脱离版本直接修改清单树。
...
### Requirement: 清单页面必须显式展示版本和来源

清单页面必须显式展示当前阶段、当前版本号、来源版本、锁定状态、创建人和变更原因。
...
### Requirement: 清单树必须支持层级、排序和来源追溯

清单明细必须支持树形层级、同级排序和来源追溯，不能仅以平铺列表表达正式业务结构。
...
### Requirement: 导入、引用和结算生成必须形成不同来源语义

清单版本应能区分导入生成、上游引用、合同基线生成、过程变更和竣工结算生成等不同来源语义。
...
### Requirement: 清单提交前必须执行核心校验

清单提交前至少必须校验编号唯一性、核心字段完整性、特征或计算规则完整性、价格来源依据和过程单据绑定关系。
