...
### Requirement: 系统必须至少区分dev、test、prod环境

系统部署必须至少区分开发、测试和生产三套环境，且环境间应保持明确隔离。
...
### Requirement: V1部署必须优先追求稳定和可维护

V1 的部署设计必须优先满足快速搭建、稳定联调、便于排障和可持续维护，而不是追求过早复杂化。
...
### Requirement: 业务数据、文件实体和临时状态必须分开存放
...
### Requirement: Frontend、API和Worker必须作为独立运行角色部署

系统必须将前端、业务 API 和 Worker 视为独立运行角色，避免把异步任务和交互服务完全混为同一运行边界。
...
### Requirement: V1的流程引擎可以内嵌于API运行边界

在 V1 阶段，流程引擎可以作为 API 内部模块运行，而不强制要求独立拆分成单独服务。
...
### Requirement: 开发环境必须支持轻量本地联调

开发环境应支持基于 Docker Compose 或等价轻量方案的一键启动，方便本地开发和单人联调。
...
### Requirement: 测试与生产环境不得共用关键存储

测试环境和生产环境不得共用数据库、缓存、对象存储 bucket 或其他关键存储资源。
...
### Requirement: 无状态服务不得依赖本机状态

API 等无状态服务不得将 session、正式文件或关键任务状态依赖于单机本地存储。
...
### Requirement: 生产环境必须具备监控、日志和恢复能力

生产环境必须至少具备日志采集、基础监控、备份和恢复能力。
