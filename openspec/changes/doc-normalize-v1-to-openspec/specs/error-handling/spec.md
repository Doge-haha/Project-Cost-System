...
### Requirement: MUST API错误必须采用统一响应结构

MUST 系统 API 错误响应必须至少包含错误码和错误消息，并允许携带结构化细节，供前端展示、日志追踪和协作排查使用。
...
### Requirement: MUST HTTP状态码必须表达错误类别

MUST 系统必须用 HTTP 状态码表达错误类别，避免全部返回 200 后再由业务字段区分失败。
...
### Requirement: MUST 参数错误和业务规则错误必须区分

MUST 参数格式错误、字段缺失与业务规则不满足必须区分处理。参数错误可返回 400，字段格式正确但业务规则不满足时应返回 422。
...
### Requirement: MUST 锁定资源写入必须返回锁定语义

MUST 当数据处于锁定状态且不允许直接修改时，写接口必须返回锁定语义，优先使用 423 表达资源被锁定。
...
### Requirement: MUST 前端反馈级别必须与错误类别匹配

MUST 前端 toast、alert 或页面错误卡片必须根据错误类别选择反馈级别：成功为 success，参数或冲突类问题为 warning，认证、权限和服务异常为 error。
...
### Requirement: MUST 表格行级错误必须可定位

MUST 表格编辑、计算、导入预览等行级交互必须能将错误定位到具体行或字段，不得只显示无法定位的全局错误。
...
### Requirement: MUST 任务型错误必须保留失败上下文

MUST 批量导入、批量重算、报表导出和 AI 推荐等任务型能力失败时，必须保留失败上下文，支持查看失败原因、失败范围和后续重试或重新发起。
...
### Requirement: MUST AI失败不得阻断人工主流程

MUST AI 推荐、AI 审核、知识抽取等辅助能力失败时，不得阻断人工主流程，应提示稍后重试或切换手工处理。
...
### Requirement: MUST 服务端异常不得泄露敏感内部细节

MUST 生产环境中的 500 类错误不得直接暴露堆栈、数据库连接串、密钥、对象存储地址签名等敏感信息。
