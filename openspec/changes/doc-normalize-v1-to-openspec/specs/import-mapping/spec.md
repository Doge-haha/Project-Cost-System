...
### Requirement: 导入必须优先保留源系统主键和编码

系统在导入旧系统数据时，必须优先保留源系统主键、编码、版本字段和上下文标识，避免后续无法追溯来源。
...
### Requirement: 不同语义的源字段不能被粗暴合并

系统不得把语义不同的源字段强行合并为同一个目标字段，除非该合并仅用于展示且不会破坏业务语义。
...
### Requirement: 无法完全映射的源字段必须有保留落点

当源字段暂时无法完全纳入 V1 业务模型时，系统必须为其提供保留落点，如预留字段或原始载荷字段，而不是直接丢弃。
...
### Requirement: 专业、定额集和清单规范版本必须分开建模

系统必须将专业、定额集和清单规范版本视为不同建模维度，不得混为单一编码概念。
...
### Requirement: 清单导入必须保留版本与来源上下文

导入清单集合、清单项和工作内容时，系统必须保留清单规范版本、源清单 ID、源序号和来源层级等关键上下文。
...
### Requirement: 工作内容必须允许作为独立子表导入

若源系统将工作内容建模为独立子表，系统必须允许其以独立从属结构导入，而不是全部塞入单个文本字段。
...
### Requirement: 参考价格不能直接当作正式计价结果

源系统中的参考单价、标准价格或类似字段，只能作为参考来源，不得在导入时直接覆盖为正式业务价格。
...
### Requirement: 导入必须生成当前阶段的初始版本

当用户以 Excel、XML、GBQ 或其他源格式导入业务数据时，系统必须在当前阶段生成可追溯的初始版本。
...
### Requirement: 导入动作必须受权限和审计控制

源数据导入必须受独立 `import` 权限约束，并保留导入人、导入时间和导入摘要等审计信息。
