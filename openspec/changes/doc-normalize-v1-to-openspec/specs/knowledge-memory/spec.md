...
### Requirement: 系统必须区分知识与记忆

系统必须明确区分“知识”和“记忆”两类沉淀对象。
...
### Requirement: 知识与记忆不得混写

系统不得把通用知识和局部记忆混在同一沉淀层中，以避免普适规律被局部偏好污染。
...
### Requirement: 知识库必须同时支持文档型和结构化沉淀

系统必须同时支持文档型知识和结构化知识，以适配检索增强、规则搜索、统计分析和模式推断。
...
### Requirement: 记忆必须能够按主体与来源追溯

每条记忆应至少能追溯到主体、来源动作或来源任务，避免形成不可解释的黑盒偏好。
...
### Requirement: 知识与记忆必须可供 MCP 和 AI 使用

系统沉淀的知识与记忆必须能够以受控方式被 MCP、AI Runtime 或相关查询能力读取，用于检索、辅助推理和上下文增强。
...
### Requirement: 知识与记忆读取必须受权限控制

知识和记忆虽然服务 AI，但其读取仍必须受项目权限、资源边界和数据可见性限制。
...
### Requirement: 知识与记忆的生成和使用必须可审计

知识候选提取、记忆生成、查询命中和关键使用动作应可追溯，以支持治理和质量提升。
