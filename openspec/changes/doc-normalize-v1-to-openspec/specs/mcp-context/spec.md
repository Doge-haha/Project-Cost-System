...
### Requirement: MUST MCP层必须作为AI专用能力层存在

MUST 系统必须将 MCP 层视为面向 AI Agent 的能力接口层，而不是简单复用前后端联调 API。
...
### Requirement: MUST MCP层不得绕过业务服务直接查库

MUST MCP 层不得绕过业务服务和权限模型直接访问数据库表。
...
### Requirement: MUST MCP能力必须区分resource和tool
...
### Requirement: MUST 系统必须支持分层上下文资源

MUST MCP 层至少应支持以下上下文资源方向：
...
### Requirement: MUST MCP上下文必须以任务友好的聚合结果返回

MUST MCP 返回的上下文必须优先面向任务理解和推理，而不是原始分散表结构。
...
### Requirement: MUST MCP动作型工具必须受更严格权限控制

MUST 所有动作型 MCP 工具必须在查询和资源读取基础上追加更严格的权限控制，尤其是涉及写操作和预览执行时。
...
### Requirement: MUST MCP返回必须经过权限裁剪

MUST MCP 层返回的资源和工具结果必须经过权限裁剪，只暴露调用者被允许看到的内容。
...
### Requirement: MUST MCP上下文必须允许接入知识与记忆增强

MUST MCP 上下文应允许在权限满足时引入知识条目、记忆摘要和历史案例，用于增强 AI 推理质量。
...
- `export-summary-report` 工具当前支持透传报表模板 ID 与输出格式，和 API 报表导出任务 payload 口径保持一致。
