...
### Requirement: MUST 大文件实体必须走对象存储而不是业务主表

MUST 报表文件、导入文件、错误报告和其他大文件实体必须走对象存储，业务数据库只保留元数据和引用关系。
...
### Requirement: MUST 文件元数据必须绑定明确业务上下文

MUST 每个正式文件记录都必须绑定所属项目、任务类型、来源任务或业务对象，以便后续追溯和权限控制。
...
### Requirement: MUST 对象存储必须默认采用私有访问边界

MUST 对象存储中的正式业务文件必须默认采用私有访问边界，而不是直接公开暴露。
...
### Requirement: MUST 开发与生产环境必须支持不同对象存储实现

MUST 对象存储能力应支持本地私有化环境和云环境的不同实现，例如本地 MinIO 与云上 OSS/S3/COS。
