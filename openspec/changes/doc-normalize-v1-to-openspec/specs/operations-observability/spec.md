...
### Requirement: MUST 系统必须区分应用日志、审计日志和任务日志

MUST 系统日志必须至少区分应用日志、审计日志和任务日志。三类日志服务于不同目的，不得互相替代。
...
### Requirement: MUST 应用日志必须覆盖请求耗时和异常信息

MUST 应用日志必须覆盖请求耗时、异常摘要、外部依赖调用失败和慢查询等排障关键事实。
...
### Requirement: MUST 生产环境必须提供健康检查

MUST 生产环境必须提供健康检查能力，用于判断 API、Worker 和关键依赖是否可用。
...
### Requirement: MUST 监控指标必须覆盖核心运行组件

MUST 监控指标必须至少覆盖应用存活、数据库连接、Redis、后台任务成功失败数、报表导出耗时和 AI 调用失败率。
...
### Requirement: MUST 关键任务失败必须触发告警

MUST 导入、导出、重算、知识抽取和 AI 推荐等关键任务失败达到阈值时，系统必须触发告警或进入可见的运维待处理状态。
...
### Requirement: MUST 错误日志必须集中收集

MUST 生产环境错误日志必须集中收集，避免只保存在单台实例本地。
...
### Requirement: MUST 敏感信息不得进入普通日志

MUST 日志不得记录明文密钥、JWT、数据库连接串、对象存储签名 URL、用户敏感字段等信息。
...
### Requirement: MUST 生产环境必须具备备份与恢复演练

MUST 生产环境必须至少具备数据库备份、对象存储备份、配置备份和周期性恢复演练。
...
### Requirement: MUST 观测能力必须支持跨组件关联

MUST API、Worker、MCP Gateway、AI Runtime 和对象存储相关操作应尽量通过请求 id、任务 id、项目 id 或 trace id 关联，支持跨组件排障。
