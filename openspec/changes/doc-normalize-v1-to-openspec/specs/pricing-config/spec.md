...
### Requirement: MUST 项目必须支持绑定默认价目版本和默认取费模板

MUST 项目层必须支持绑定默认价目版本和默认取费模板，作为后续计价和页面展示的默认配置来源。
...
### Requirement: MUST 价目版本必须支持按地区和专业筛选

MUST 价目版本查询必须至少支持按地区、专业和状态筛选，以匹配项目实际取价场景。
...
### Requirement: MUST 取费模板必须支持按地区、项目类型和阶段筛选

MUST 取费模板查询必须至少支持按地区、项目类型、阶段和状态筛选，以命中正确计费规则。
...
### Requirement: MUST 切换价目版本或取费模板必须触发结果重算

MUST 当项目或阶段切换价目版本或取费模板时，系统必须触发受影响结果重算或进入待重算状态。
...
### Requirement: MUST 取费规则命中必须遵循明确优先级

MUST 当存在多条可能命中的取费规则时，系统必须按明确优先级选择生效规则，并在冲突时提示确认。
...
### Requirement: MUST 已被引用的模板不得被直接删除覆盖

MUST 已被项目引用的价目版本或取费模板不得被直接删除或无痕覆盖，应通过停用或生成新版本方式维护。
