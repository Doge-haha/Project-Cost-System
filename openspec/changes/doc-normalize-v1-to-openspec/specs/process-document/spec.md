...
### Requirement: 过程单据必须作为施工过程阶段的正式业务对象

变更单、签证单、进度款等过程单据必须作为施工过程中的正式业务对象管理，不能只以备注或临时字段方式存在。
...
### Requirement: 过程单据必须绑定明确的阶段和专业上下文

每条过程单据都必须绑定所属项目、阶段和专业上下文，确保可见范围、责任归属和后续联动一致。
...
### Requirement: 过程单据必须支持草稿、提交、通过和驳回状态

过程单据必须具备至少草稿、已提交、已通过和已驳回等状态语义，用于支撑编制、审核和结果生效。
...
### Requirement: 审批通过的过程单据才能影响正式结果

只有审批通过的过程单据才允许影响清单版本、汇总金额或结算结果，未通过单据不得作为正式变动依据。
...
### Requirement: 施工过程阶段的新增或调整必须可追溯到过程单据

施工过程阶段发生新增项、调整项或重新测算时，必须能够追溯到对应过程单据。
...
### Requirement: 过程单据通过后必须触发下游联动

过程单据审批通过后，系统必须触发清单变更版本生成、结果重算或汇总刷新等下游联动，而不是停留在孤立记录层。
