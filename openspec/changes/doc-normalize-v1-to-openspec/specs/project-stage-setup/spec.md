...
### Requirement: MUST 项目创建必须以阶段模板为起点

MUST 新项目创建必须从阶段模板或自定义模板展开，而不是让阶段链路完全从空白状态手工拼装。
...
### Requirement: MUST 阶段启用和顺序必须可按项目单独调整

MUST 即使来源于模板，启用阶段、阶段顺序和阶段负责人也必须允许在项目层按需调整。
...
### Requirement: MUST 项目创建时必须初始化关键默认参数

MUST 项目创建或项目初始化时，系统必须能够绑定默认审核人、默认价目版本、默认取费模板或其他关键业务参数。
...
### Requirement: MUST 需要审核的阶段必须具备审核责任人配置

MUST 若项目启用了需要审核的阶段，则必须存在审核责任人或等价审核配置，避免流程断点。
...
### Requirement: MUST 项目创建完成后必须进入统一工作台

MUST 项目创建完成后，系统应将用户带入统一工作台，而不是停留在孤立配置表单。
