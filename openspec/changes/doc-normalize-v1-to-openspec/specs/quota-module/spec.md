...
### Requirement: MUST 定额模块必须在清单上下文中工作

MUST 定额模块必须围绕当前清单项工作，不能脱离清单上下文孤立进行正式定额处理。
...
### Requirement: MUST 定额页面必须展示价目版本和来源方式

MUST 定额页面必须能够展示当前定额所使用的价目版本、所属地区和来源方式。
...
### Requirement: MUST 定额选择器必须支持按专业、定额集和地区查询

MUST 定额选择器必须支持至少按专业、定额集、地区和关键字查询候选定额。
...
### Requirement: MUST 套用校验必须覆盖缺定额、单位不一致和异常含量

MUST 定额模块必须至少校验缺定额、单位不一致、异常含量、负数费用以及过程单据绑定等核心问题。
...
### Requirement: MUST AI定额推荐必须经过人工确认

MUST AI 生成的定额候选只能作为推荐结果，必须经过人工筛选和确认后才能成为正式定额套用结果。
...
### Requirement: MUST 参考定额知识库必须作为只读候选来源

MUST 外部定额数据资产、向量索引和历史套用样本可以补充定额候选召回，但必须以只读参考库接入，不得绕过人工确认直接写入正式 `quota_line`。
...
### Requirement: MUST 定额候选必须保留可解释匹配信息

MUST 定额候选结果必须说明命中方式和关键依据，至少覆盖关键字命中、默认定额集候选、历史引用或语义召回等来源说明。
