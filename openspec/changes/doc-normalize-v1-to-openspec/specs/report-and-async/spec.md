...
### Requirement: MUST 报表必须绑定明确的数据上下文

MUST 每个报表结果都必须绑定明确的数据上下文，至少包括所属项目、所属阶段或过滤维度、模板来源和生成版本。
...
### Requirement: MUST 内置模板不得被直接覆盖

MUST 系统内置的国标报表模板不得被直接覆盖，只能复制后生成企业模板或自定义模板。
...
### Requirement: MUST 模板字段映射必须引用可追溯标准字段

MUST 报表模板的字段映射必须引用系统可追溯的标准字段，不得依赖无法追溯来源的手工拼装值作为正式字段来源。
...
### Requirement: MUST 报表导出默认应采用异步任务模式

MUST 报表生成和导出默认应以异步任务方式执行，以支持排队、处理、完成、失败和后续下载。
...
### Requirement: MUST 异步导出任务必须具备完整状态反馈

MUST 导出任务至少必须支持排队中、处理中、已完成和失败等状态语义，并允许用户查询当前状态。
...
### Requirement: MUST 仅已完成任务允许下载

MUST 只有已经完成的导出任务才允许进入下载流程，未完成或失败任务不得伪装成可下载结果。
...
### Requirement: MUST 失败任务必须保留错误信息并允许重发

MUST 失败的报表任务或后台任务必须保留错误原因，并允许用户或系统重新发起同类任务。
...
### Requirement: MUST 后台任务必须具备统一追踪口径

MUST 除报表导出外，系统中的后台任务也必须统一采用可追踪的任务模型，至少记录任务类型、请求人、状态、创建时间、完成时间、结果和错误信息。
...
### Requirement: MUST 报表与后台任务必须受权限控制

MUST 报表导出和后台任务查询必须受到项目可见性和导出权限控制，不得对无权用户暴露任务内容或结果。
...
### Requirement: MUST 关键任务状态切换必须写入审计日志

MUST 导出任务和后台任务的关键状态切换至少应对排队、完成和失败进行审计记录。
