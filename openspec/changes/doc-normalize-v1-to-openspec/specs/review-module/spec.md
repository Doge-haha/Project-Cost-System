...
### Requirement: MUST 正式成果必须通过审核提交流进入审核流

MUST 清单版本、阶段成果或其他正式业务成果在进入审核前，必须先形成明确的审核提交流对象，而不是直接修改资源终态。
...
### Requirement: MUST 审核流必须支持待审核、通过、驳回和撤回

MUST 审核流必须至少支持待审核、通过、驳回和撤回等状态语义，以承载完整的审核决策过程。
...
### Requirement: MUST 审核人与提交人必须分离

MUST 审核人不得审核自己提交的成果，系统必须显式落实审编分离。
...
### Requirement: MUST 同一资源不得并存多个待审核记录

MUST 同一正式资源在已有待审核记录时，不得重复提交新的待审核记录，以避免状态冲突。
...
### Requirement: MUST 审核通过或驳回必须联动资源状态

MUST 审核通过或驳回后，系统必须联动更新对应资源、阶段或项目状态，避免出现审核结论与业务状态脱节。
...
### Requirement: MUST 撤回只允许发生在未审核终态前

MUST 撤回动作只能发生在待审核阶段，已经通过或驳回的记录不得再被撤回改写。
