...
### Requirement: 源系统专业必须视为多层概念而非单一枚举

源系统中的“专业”必须视为至少包含标准专业主数据和业务视图专业两层概念，不能简单压平为单一名称字段。
...
### Requirement: 专业与定额集必须分开兼容

兼容层必须将专业与定额集视为不同维度，不得因为源系统中存在映射关系就将其合并为一个概念。
...
### Requirement: Markup等源编码必须保留但不应直接视为稳定主键

像 `Markup` 这类源系统编码必须保留用于兼容和追溯，但不应未经处理直接作为长期稳定主键。
...
### Requirement: 源系统清单字段必须保留层级和规范语义

源系统中的清单编号、层级码、规范版本码和源 ID 等字段必须保留，以维持高保真兼容与追溯能力。
...
### Requirement: 源系统工作内容必须允许独立表达

当源系统中工作内容是独立结构时，兼容层必须允许其在目标系统中独立表达，而不是只能退化为单一文本。
...
### Requirement: 源系统兼容必须优先保留确定信息，避免过度推断

兼容专题文档和兼容实现都必须优先固化已确认信息，不应把尚未验证的字段语义过度推断为正式规则。
