...
### Requirement: MUST 核心对象必须存在单一主状态语义

MUST 每个核心业务对象必须存在一套单一主状态语义，不应让同一流程被多个互相冲突的状态字段重复表达。
...
### Requirement: MUST 状态变化必须由显式事件触发

MUST 系统中的状态变化必须由提交、审核、锁定、解锁、归档等显式事件触发，不允许无依据的隐式跳转。
...
### Requirement: MUST 驳回后应回到最近可编辑状态

MUST 被驳回的业务对象原则上必须回到最近可编辑状态，而不是进入额外的模糊半状态。
...
### Requirement: MUST 项目必须具备基本生命周期状态

MUST 项目至少必须支持以下生命周期语义：
...
### Requirement: MUST 清单版本必须具备独立审核状态

MUST 清单版本必须至少支持以下状态语义：
...
### Requirement: MUST 锁定与解锁必须具备独立流程语义

MUST 锁定与解锁必须被视为独立流程，至少应支持以下语义：
...
### Requirement: MUST 审核提交流必须具备独立记录状态

MUST 审核提交流必须作为独立记录管理，至少支持以下状态语义：
...
### Requirement: MUST 过程单据必须仅在审批通过后生效

MUST 变更单、签证单、进度款等过程单据必须至少支持草稿、待审批、已通过、已驳回和已结清或已关闭等语义，并且只有审批通过后才能生效。
...
### Requirement: MUST 异步任务必须具备可追踪执行状态

MUST 报表导出、后台任务或类似异步处理必须具备排队中、处理中、已完成和失败等基础状态。
...
### Requirement: MUST 状态联动必须保持一致性

MUST 项目、阶段、清单版本、审核提交和锁定状态之间必须遵守一致的联动规则。
...
### Requirement: MUST 所有关键状态切换必须可审计

MUST 以下关键状态切换必须写入审计记录：
