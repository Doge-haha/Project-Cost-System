...
### Requirement: 汇总页必须支持多维度查看结果

汇总页必须至少支持按项目、阶段、专业、单体和费用结构等维度查看结果，不得将正式汇总能力限制为单一视角。
...
### Requirement: 汇总结果必须基于明确版本上下文

每次汇总和分析都必须绑定明确的数据版本、筛选条件和统计范围，以保证结果可解释和可追溯。
...
### Requirement: 偏差分析必须支持核心阶段对比

系统必须支持估算、目标、合同、过程和结算等核心阶段之间的偏差分析，用于识别成本偏离。
...
### Requirement: 超阈值偏差必须形成风险提示

当偏差超过预设阈值时，系统必须在汇总视图中形成风险提示，并允许用户下钻定位来源。
...
### Requirement: 汇总分析必须支持版本对比

汇总分析必须支持对两个版本进行同口径对比，以帮助识别引用、变更或重算带来的结果差异。
...
### Requirement: 汇总指标必须区分系统结果与最终结果

若系统存在人工调价或人工修正，汇总指标必须能够区分系统结果和最终结果，避免偏差分析失真。
