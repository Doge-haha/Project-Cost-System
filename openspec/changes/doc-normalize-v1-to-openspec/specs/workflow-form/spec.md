...
### Requirement: 流程引擎只负责流转，业务系统掌握业务真相

系统必须坚持流程引擎只负责“流程流转”，业务系统继续掌握业务主状态和业务事实。
...
### Requirement: 清单和定额主编辑界面不得强行走通用表单引擎

清单、定额、汇总和报表等重表格业务不得强行放入通用流程表单渲染器，必须保留领域专用渲染体系。
...
### Requirement: V1只支持可配置流程和可配置表单，不扩展成通用低代码平台

V1 应支持流程模板、节点配置、审批人规则、节点表单配置和字段显隐只读必填等能力，但不应扩展成通用低代码平台。
...
### Requirement: 系统必须支持流程定义、版本、绑定和运行时实例分层
...
### Requirement: 流程绑定必须支持按业务条件命中

流程绑定必须允许至少按 `resource_type`、`stage_code`、`submission_type`、`project_type` 等维度命中合适流程。
...
### Requirement: 表单配置必须拆分数据结构、UI布局和字段规则
...
### Requirement: 节点级字段权限必须可配置

系统必须允许在流程节点级别配置字段的显隐、只读、必填等规则。
...
### Requirement: 流程动作必须形成表单提交与任务记录

用户在流程中的提交、通过、驳回、撤回等动作必须形成可追溯的任务记录和表单提交记录。
